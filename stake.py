...
def check_position_approval(token_id, approval_address):
    """Check if a position is approved for the specified address"""
    try:
...
def approve_position(token_id, approval_address, approval_name=""):
    """Approve the position for a specific address"""
    try:
        # Check if already approved
        if check_position_approval(token_id, approval_address):
            print(f"Position {token_id} is already approved for {approval_name}")
            return True
            
        print(f"Approving position {token_id} for {approval_name}...")
        ...
        # Build transaction
        tx = npm_contract.functions.approve(approval_address, token_id).build_transaction({
            'from': wallet_address,
            'gas': 120000,
            'gasPrice': gas_price,
            'nonce': nonce,
        ...
        if receipt.status == 1:
            print(f"Position {token_id} approval for {approval_name} successful!")
            return True
        else:
            print(f"Failed to approve position {token_id} for {approval_name}")
            return False
...
def stake_position(token_id):
    ...
    try:
        ...
        if receipt.status == 1:
            ...
            # Check if we can get more info about the failure
            print("Please check the transaction on BaseScan for more details.")
            return False
            
    except ValueError as e:
        error_str = str(e)
        print(f"Error in staking: {error_str}")
